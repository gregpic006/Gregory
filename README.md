...
**Backend** : 30 Supabase Edge Functions (Deno/TypeScript), chacune un fichier `edge-function-<nom>.ts` à la racine — le nom de la fonction déployée sur Supabase est `<nom>` (sans le préfixe `edge-function-` ni le `.ts`). Deux familles :
...
- `confirmer-reparation.html`, `confirmer-visite.html`, `reponse-travailleur.html` — pages de confirmation à token à usage unique (pas de compte requis)
...
## État connu du projet (à la dernière session)

Un audit de sécurité/fonctionnel a identifié des points non encore corrigés :
- **Sécurité** : `flinks-api.ts` action `sync_all` sans authentification ; les cascades automatiques de réassignation de travailleur (`process_worker_response_timeouts()` dans schema.sql, et `handle-worker-response.ts`) ne filtrent pas par vérification RBQ/assurance/statut Portail Pro ; policy RLS `workers` trop large (tout propriétaire voit tous les travailleurs) ; aucune fonction edge ne vérifie elle-même la signature JWT.
- **Fonctionnel** : risque de collision sur `invoice_number` lors de la génération concurrente des factures mensuelles (`generate-owner-report.ts`) sans vérification du résultat de l'insert.
